...
const Issue = ({ api }) => {
  ...
  const newLink = api.replace(
    "https://comicvine.gamespot.com/api",
    "https://batserver.vercel.app/comicvine"
  );
  const url = newLink.concat(
    "field_list=person_credits,image,description,volume"
  );
  ...
  return (
    <div className="issue" onClick={turnIssue}>
      <div
        className="front"
        ref={front}
        style={{ backgroundImage: `url(${data.results.image.small_url})` }}
      ></div>
      <div className="back" ref={back}>
        <div
          className="issue--info"
          dangerouslySetInnerHTML={{ __html: data.results.description }}
        ></div>
        <ul>{list}</ul>
        <Link to={`/comics/vol_${volumeID}`}>
          <button className="issue--link">Check the full Volume!</button>
        </Link>
      </div>
    </div>
  );
